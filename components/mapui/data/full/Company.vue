<template>
  <div class="mb-16 p-2">
    <div class="mb-6 px-5">
      <h1 class="mb-2 text-2xl font-bold">{{ element.name }}</h1>
      <h3>{{ element.address }}</h3>
      <p>Siret : {{ element.siret }}</p>
      ...
      <p>{{ element.naf_text }}</p>
      <p>Contact : {{ element.contact_mode }}</p>
    </div>
    <hr class="mx-auto h-1 w-96 bg-gray-light" />
    <div class="mb-6 mt-6 px-5">
      <p>Naf : {{ element.naf_text }}</p>
      <p>Code rome : {{ element.matched_rome_code }}</p>
      <p>Label rome : {{ element.matched_rome_label }}</p>
      <p class="m-4 bg-brand-primary-light p-4">
        {{ element.headcount_text }}
      </p>
    </div>
  </div>
</template>
